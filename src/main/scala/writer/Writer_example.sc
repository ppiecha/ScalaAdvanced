...
object Client {
  def makeRaw(id: Long, name: String, age: Int): Tracked[Client] =
    Client(id, name, age).writer(List(s"Client created $id $name $age"))
}

case class Product(id: Long, name: String, unitPrice: Double)
object Product {
  def makeRaw(id: Long, name: String, unitPrice: Double): Tracked[Product] =
    Product(id, name, unitPrice).writer(List(s"Product created $id $name $unitPrice"))
}
...
object ShoppingCart {
  ...
  def makeRaw(
    ...
    val client = Client.makeRaw(clientId, clientName, clientAge).value
    val cardItems = items.map(ShoppingCartItem.makeRaw).map(_.value)
    ShoppingCart(client, cardItems).writer(List(s"ShoppingCart created $client $cardItems"))
}
